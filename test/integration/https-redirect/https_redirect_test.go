...
func TestHttpsRedirect(t *testing.T) {
	...
	bpt.DefineVerify(func(assert *assert.Assertions) {
		...
		test.AssertResponseStatus(t, assert, "http://"+loadBalancerIp, 301)

		insecureClient := &http.Client{Transport: &http.Transport{TLSClientConfig: &tls.Config{InsecureSkipVerify: true}}}
		test.AssertResponseStatusWithClient(t, assert, insecureClient, "https://"+loadBalancerIp, 200)
	})
